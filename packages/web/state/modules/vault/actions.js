...
export const performVaultItemEncryption = ({
    overview,
    details,
    vaultKey,
    email,
...
export const performVaultArchiveDecryption = ({ encArchiveList, vaultKey }) => async dispatch => {
    try {
        ...
        // Array to object
        const decArchiveObjectList = Object.assign({}, ...decVaultData.map(item => ({ [item.itemId]: item })));
        if (decVaultStatus) {
...
export const deleteVaultItem = ({ itemId }) => async dispatch => {
    try {
        const { data } = await api({
            method: 'POST',
